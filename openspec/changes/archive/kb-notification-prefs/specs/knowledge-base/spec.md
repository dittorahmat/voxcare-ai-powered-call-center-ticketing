...
- **WHEN** admin POSTs `{ title: "Cara Reset Password", content: "...", category: "Akun", tags: ["password", "login"], published: true }`
...
- **WHEN** user GETs `/api/knowledge-base/articles?category=Akun`
- **THEN** only published articles in the "Akun" category are returned
...
- **WHEN** customer POSTs a new ticket with `{ title: "Tidak bisa login ke akun", ... }`
- **THEN** the response includes a `suggestedArticles` array with matching KB articles (e.g., "Cara Reset Password", "Kebijakan Penguncian Akun")
