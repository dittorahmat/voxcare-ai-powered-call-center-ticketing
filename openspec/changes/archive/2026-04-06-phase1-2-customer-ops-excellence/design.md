...
**Revised Decision:** Use Cloudflare **Browser Rendering API** for server-side PDF generation. The worker endpoint sends HTML to the Browser Rendering API (`https://browser-rendering.cloudflare.com`), which renders it in a headless browser and returns a PDF blob. For client-side PDF download in the web UI, the existing `@react-pdf/renderer` components work fine in the browser.

**Rationale:**
- `@react-pdf/renderer` **does not work** on Cloudflare Workers out-of-the-box because its `yoga-layout` dependency uses WASM, which is blocked by Cloudflare's security model
- A patch exists (swap to `yoga-layout/asmjs-sync`) but is fragile and breaks on dependency updates
- Cloudflare's Browser Rendering API is a managed headless browser service — reliable, no WASM issues, supports full CSS/HTML rendering
- The API is already documented and supported by Cloudflare
- Scheduled reports: HTML email body with "Download PDF" link pointing to the web UI (client-side generation)

**Alternatives considered:**
- *Patch @react-pdf/renderer*: Fragile, breaks on `yoga-layout` updates
- *HTML-only scheduled reports*: No downloadable PDF at all, least flexible
...
**Language:** Articles are in **Bahasa Indonesia only** for this phase. Multi-language support (article versioning) is deferred to Phase 3.
...
**UX for suggested responses:** The AI draft is shown in a **separate suggestion panel** (not auto-inserted into the reply box). The agent must explicitly click "Use" to copy it into the reply editor, or "Dismiss" to ignore it. This maintains agent control and prevents accidental AI-generated responses being sent.
...
|------|--------|------------|
| **PDF generation requires Browser Rendering API** | Needs separate API setup | Cloudflare managed service, well-documented. Add API key to wrangler.jsonc. |
...
| **Browser Rendering API rate limits** | PDF generation may throttle under heavy load | Cache PDF results, use rate limiting on PDF endpoint. Monitor usage. |
...
1. **Should the knowledge base support multiple languages?**
   - **DECIDED:** Single language — **Bahasa Indonesia** only for this phase. Multi-language support (article versioning) deferred to Phase 3.

2. **Should AI suggested responses be inserted directly into the reply box, or shown as a separate panel?**
   - **DECIDED:** **Separate suggestion panel.** Agent must click "Use" to copy into reply editor, or "Dismiss" to ignore. Maintains agent control.

3. **Should sentiment scores trigger automatic priority escalation, or just flag/warn the agent?**
   - **DECIDED:** **Flag/warn only** for Phase 2. Auto-escalation via sentiment reserved for Phase 3 (workflow automation builder).

4. **PDF report generation on Workers — how to handle @react-pdf/renderer WASM incompatibility?**
   - **RESOLVED:** Use Cloudflare **Browser Rendering API** for server-side PDF. Well-documented, managed service, no WASM issues.
